...
- Yours page (`/yours`, connect or type an address, `/go` redirect), holder pages with one row per day and downloads (SVG from the server, PNG and JPEG drawn in the browser at 1024/2048/4096), breadcrumb to the hub on every page.
...
5. **Mobile wallets.** WalletConnect needs a project id; injected wallets only for now. Typing an address on `/yours` covers phones without one.

...
- `bun test` (site, 45 tests) · `cd contracts && forge test` (19 tests, needs `via_ir`) · `bun run contracts/fixtures.ts` regenerates the byte-equality fixtures after any change to `src/knot.ts` (then the Solidity test must still pass). Never touch `src/knot_v2.ts` or `contracts/src/KnotRenderer.sol`.

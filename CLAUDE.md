...
- The image is generated fully on-chain by a separate renderer contract from the day number alone. The TypeScript in `src/knot.ts` and the Solidity in `contracts/src/KnotRenderer.sol` produce the same SVG byte for byte; a Foundry test enforces that against fixtures generated from TS. **TypeScript is the source of truth.**
...
- `bun test` (site, 31 tests) · `cd contracts && forge test` (16 tests) · `bun run contracts/fixtures.ts` regenerates the byte-equality fixtures after any change to `src/knot.ts` (then the Solidity test must still pass).
...
- `contracts/deploy.sh sepolia|mainnet` deploys renderer + token (Sourcify verification). `contracts/wire.sh sepolia|mainnet` writes the deployed addresses into the hosting env and redeploys the site. `scripts/status.sh` prints on-chain and site state.
